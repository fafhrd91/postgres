...
/// All `ToSql` implementations should use this macro.
#[macro_export]
macro_rules! to_sql_checked_vec {
    () => {
...
#[doc(hidden)]
pub fn __to_sql_checked<T>(v: &T, ty: &Type, out: &mut BytesMut) -> Result<IsNull, Box<dyn Error>>
where
    T: ToSql,
{
    if !T::accepts(ty) {
...
pub trait ToSql: fmt::Debug {
    ...
    /// anything to `out`.
    fn to_sql(&self, ty: &Type, out: &mut BytesMut) -> Result<IsNull, Box<dyn Error>>
    where
        Self: Sized;
...
impl<'a, T: ToSql> ToSql for &'a [T] {
    fn to_sql(&self, ty: &Type, w: &mut BytesMut) -> Result<IsNull, Box<dyn Error>> {
        let member_type = match *ty.kind() {
            Kind::Array(ref member) => member,
            _ => panic!("expected array type"),
        };
    ...
    fn to_sql_vec(&self, ty: &Type, w: &mut BytesVec) -> Result<IsNull, Box<dyn Error>> {
        ...
        types::array_to_sql_vec(
            Some(dimension),
            member_type.oid(),
            self.iter(),
            |e, w| match e.to_sql_vec(member_type, w)? {
                IsNull::No => Ok(postgres_protocol::IsNull::No),
                IsNull::Yes => Ok(postgres_protocol::IsNull::Yes),
            },
            w,
...
impl<'a> ToSql for &'a [u8] {
    fn to_sql(&self, _: &Type, w: &mut BytesMut) -> Result<IsNull, Box<dyn Error>> {
...
macro_rules! simple_to {
    ($t:ty, $f:ident, $f1:ident, $($expected:ident),+) => {
        impl ToSql for $t {
            fn to_sql(&self,
                      _: &Type,
                      w: &mut BytesMut)
...
simple_to!(bool, bool_to_sql, bool_to_sql_vec, BOOL);
simple_to!(i8, char_to_sql, char_to_sql_vec, CHAR);
simple_to!(i16, int2_to_sql, int2_to_sql_vec, INT2);
simple_to!(i32, int4_to_sql, int4_to_sql_vec, INT4);
simple_to!(u32, oid_to_sql, oid_to_sql_vec, OID);
simple_to!(i64, int8_to_sql, int8_to_sql_vec, INT8);
simple_to!(f32, float4_to_sql, float4_to_sql_vec, FLOAT4);
simple_to!(f64, float8_to_sql, float8_to_sql_vec, FLOAT8);
...
impl ToSql for SystemTime {
    ...
    fn to_sql_vec(&self, _: &Type, w: &mut BytesVec) -> Result<IsNull, Box<dyn Error>> {
        ...
        types::timestamp_to_sql_vec(time, w);
        Ok(IsNull::No)
    }
...
fn downcast(len: usize) -> Result<i32, Box<dyn Error>> {
